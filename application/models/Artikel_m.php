...
class Artikel_m extends CI_Model
{
    ...
    {
        $ids = $this->db->get($this->_table)->row()->id;
    ...
    public function update()
    {
        ...
        // mengambil data id dengan method row() untuk memperoleh objek stdClass
        $ids = $this->db->get($this->_table)->row();

        // akses properti id pada objek $ids dengan operator ->
        $id = $ids->id;
        ...
        $this->db->update($this->_table, $artikel, ['id' => $id]);
    }
    ...
    private function _deleteImage($id)
    {
        ...
        if ($image['image'] != 'default.jpg') {
            ...
            return array_map(
                'unlink',
                glob(FCPATH . "upload_file/images/$filename.*")
            );
        }
    }
